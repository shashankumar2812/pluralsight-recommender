...
def run_similar_user_generator(gen_model, params, data_loader, model_handle):
    ...
    try: 
        ...
        for user_handle in data.user_handle.unique():
            count+=1
            similar_users=model.predict_similar_users(user_handle=user_handle).similar_users.tolist()
            user = UserListModel(user_handle=int(user_handle), model_handle=model_handle, similar_users=similar_users)
            user.save()
            if count%1000==0:
